...
import { rasterizeCertificate } from '@/lib/captureCertificate'
import { buildShareText, getShareLinks } from '@/lib/shareUtils'
...
function IconMessenger() {
  return (
    <svg width={24} height={24} viewBox="0 0 24 24" aria-hidden>
      <path
        fill="currentColor"
        d="M0 11.5C0 5.149 5.149 0 11.5 0S23 5.149 23 11.5c0 5.5-3.9 10.1-9.1 11.2v-7.9h2.5V11.5H13.9v-2c0-1.5.9-2.3 2.2-2.3.6 0 1.2.1 1.2.1v2.1h-1c-1 0-1.3.6-1.3 1.3v1.7h2.5l-.4 2.6h-2.1V23C6.4 21.9 0 17.3 0 11.5z"
      />
    </svg>
  )
}
...
> = {
  ...
  linkedin: { bg: 'rgba(10,102,194,0.15)', color: '#0A66C2', label: 'LinkedIn' },
  twitter: { bg: 'rgba(255,255,255,0.08)', color: '#E7E9EA', label: 'Twitter/X' },
  telegram: { bg: 'rgba(34,158,217,0.15)', color: '#229ED9', label: 'Telegram' },
...
export default function ShareModal({
  ...
  const shareText = buildShareText(recipientName, achievement)
  const links = getShareLinks(shareUrl, shareText)
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      ;(async () => {
        const el = document.getElementById(certificateId)
        if (!el || cancelled) return
        try {
          const dataUrl = await rasterizeCertificate(el, 1.5)
  ...
  const canNative =
    ...
    Boolean(navigator.canShare?.({ text: shareText, url: shareUrl }))

  const grid: { key: keyof typeof links; icon: ReactNode }[] = [
    ...
    { key: 'messenger', icon: <IconMessenger /> },
  ]
    ...
    >
      <DialogContent className="max-h-[90vh] max-w-md overflow-y-auto border-white/[0.1] bg-[#12151c] text-app sm:max-w-md">
        ...
        <div className="grid grid-cols-3 gap-2">
          {grid.map(({ key, icon }) => {
            const st = platformStyles[key]
            return (
              <button
                key={key}
                type="button"
                onClick={() => open(links[key])}
                className="flex min-h-[60px] min-w-[72px] flex-col items-center justify-center gap-1 rounded-lg py-2 transition-transform hover:scale-105"
